...
@section('title')
    @lang('server.plugins.new.header')
@endsection

@section('content-header')
    <h1>@lang('server.plugins.edit.header')<small>@lang('server.plugins.edit.header_sub')</small></h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('index') }}">@lang('strings.home')</a></li>
        <li><a href="{{ route('server.index', $server->uuidShort) }}">{{ $server->name }}</a></li>
        <li><a href="{{ route('server.subusers', $server->uuidShort) }}">@lang('navigation.server.subusers')</a></li>
        <li class="active">@lang('server.plugins.update')</li>
    </ol>
@endsection
            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                    <div class="form-group">
                        <label class="control-label">@lang('server.plugins.new.email')</label>
                        <div>
                            {!! csrf_field() !!}
                ...
                @can('edit-subuser', $server)
                    <div class="box-body">
                        ...
                        {!! method_field('PATCH') !!}
                        <input type="submit" name="submit" value="@lang('server.plugins.update')" class="pull-right btn btn-sm btn-primary" />
                    </div>
                @endcan
            </div>
        </div>
    ...
    <div class="row">
        @foreach($permlist as $block => $perms)
            <div class="col-sm-6">
                <div class="box">
                    <div class="box-header with-border">
                        <h3 class="box-title">@lang('server.plugins.new.' . $block . '_header')</h3>
                    </div>
                    <div class="box-body">
                            ...
                            <div class="form-group">
                                <div class="checkbox checkbox-primary no-margin-bottom">
                                    ...
                                    <label for="{{ $permission }}" class="strong">
                                        @lang('server.plugins.new.' . str_replace('-', '_', $permission) . '.title')
                                    </label>
                                </div>
                                <p class="text-muted small">@lang('server.plugins.new.' . str_replace('-', '_', $permission) . '.description')</p>
                            </div>
                        @endforeach
                    </div>
                </div>
